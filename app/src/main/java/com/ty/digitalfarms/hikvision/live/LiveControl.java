package com.ty.digitalfarms.hikvision.live;
...
public class LiveControl implements RtspClientCallback, PlayerDisplayCB {
    ...
    private void closePlayer() {
        if (null != mPlayerHandler) {
            if (-1 != mPlayerPort) {
                ...
                if (!ret) {
                    Log.e(TAG, "closePlayer(): Player stop  failed!  errorCode is P"
    ...
    private boolean startPlayer(byte[] data, int len) {
        ...
        if (!ret) {
            Log.e(TAG, "startPlayer() mPlayerHandle.setDisplayCB() failed errorCode is P"
                    + mPlayerHandler.getLastError(mPlayerPort));
            return false;
        }
        ...
        if (!ret) {
            Log.e(TAG, "startPlayer() mPlayerHandle.play failed!" + "Port: " + mPlayerPort + "PlayView Surface: "
