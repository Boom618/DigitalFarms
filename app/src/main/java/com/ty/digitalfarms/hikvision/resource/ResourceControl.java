 ...
 * 
 */
package com.ty.digitalfarms.hikvision.resource;
...
public final class ResourceControl {
    ...
    public void reqResList(int pType, int pId) {
        switch (pType) {
            case HikConstants.Resource.TYPE_UNKNOWN:// 第一次请求资源列表
                requestFirstList();
            break;
            case HikConstants.Resource.TYPE_CTRL_UNIT:// 从控制中心获取子资源列表
                requestSubResFromCtrlUnit(pId);
            break;
            case HikConstants.Resource.TYPE_REGION:// 从区域获取子资源列表
    ...
    private void requestSubResFromRegion(int pId) {
        ...
        String servAddr =HikConstants.SERVER_ADDRESS;
        ServInfo loginData = TempData.getIns().getLoginData();
        if(loginData == null){
            Log.i(HikConstants.LOG_TAG, "getRegionListFromRegion loginData : " + loginData);
            return;
        }
        ...
        Log.i(HikConstants.LOG_TAG, "getRegionListFromRegion ret : " + ret);
        responseFlag = responseFlag || ret;
        if (!ret) {
            Log.e(HikConstants.LOG_TAG, "Invoke VMSNetSDK.getRegionListFromRegion failed:" + errDesc());
        }
        ...
        ret = VMSNetSDK.getInstance().getCameraListFromRegion(servAddr, loginData.getSessionID(), String.valueOf(pId), numPerPage, curPage,
                cameraList);
        Log.i(HikConstants.LOG_TAG, "getCameraListFromRegion ret : " + ret);
        responseFlag = responseFlag || ret;
        if (!ret) {
            Log.e(HikConstants.LOG_TAG, "Invoke VMSNetSDK.getCameraListFromRegion failed:" + errDesc());
        }
    ...
    private void requestSubResFromCtrlUnit(int pId) {
       ...
       // String servAddr = Config.getIns().getServerAddr();
        ...
        ServInfo loginData = TempData.getIns().getLoginData();
        if(loginData == null){
            Log.i(HikConstants.LOG_TAG, "requestSubResFromCtrlUnit loginData:" + loginData);
            return;
        }
        ...
        Log.i(HikConstants.LOG_TAG, "getControlUnitList ret:" + ret);
        responseFlag = responseFlag || ret;

        if (!ret) {
            Log.e(HikConstants.LOG_TAG, "Invoke VMSNetSDK.getControlUnitList failed:" + errDesc());
        }
        List<RegionInfo> regionList = new ArrayList<RegionInfo>();
        // 2.从控制中心获取区域列表
        ret = VMSNetSDK.getInstance().getRegionListFromCtrlUnit(servAddr, sessionID, String.valueOf(controlUnitID), numPerPage,
                curPage, regionList);
        Log.i(HikConstants.LOG_TAG, "getRegionListFromCtrlUnit ret:" + ret);
        responseFlag = responseFlag || ret;
        if (!ret) {
            Log.e(HikConstants.LOG_TAG, "Invoke VMSNetSDK.getRegionListFromCtrlUnit failed:" + errDesc());
        }
        ...
        ret = VMSNetSDK.getInstance().getCameraListFromCtrlUnit(servAddr, sessionID, String.valueOf(controlUnitID), numPerPage,
                curPage, cameraList);
        Log.i(HikConstants.LOG_TAG, "getCameraListFromCtrlUnit ret:" + ret);
        responseFlag = responseFlag || ret;
        if (!ret) {
            Log.e(HikConstants.LOG_TAG, "Invoke VMSNetSDK.getCameraListFromCtrlUnit failed:" + errDesc());
        }
        ...
        Log.i(HikConstants.LOG_TAG, "allData size is " + allData.size());
        if (callback != null) {
    ...
    private void requestFirstList() {
       // String servAddr = Config.getIns().getServerAddr();
        ...
        ServInfo loginData = TempData.getIns().getLoginData();
        if (loginData == null) {
            Log.i(HikConstants.LOG_TAG, "requestFirstList loginData:" + loginData);
            return;
        }
        String sessionID = loginData.getSessionID();
        ...
        if (ctrlUnitList != null && !ctrlUnitList.isEmpty()) {
            for (ControlUnitInfo info : ctrlUnitList) {
                Log.i(HikConstants.LOG_TAG, "name:" + info.getName() + ",controlUnitID:" + info.getControlUnitID() + ",parentID:"
                        + info.getParentID());
            }
        }
        Log.i(HikConstants.LOG_TAG, "allData size is " + ctrlUnitList.size());
        if (!ret) {
            Log.e(HikConstants.LOG_TAG, "Invoke VMSNetSDK.getControlUnitList failed:" + errDesc());
        }
        if (callback != null) {
