...
public class XAxisRender extends AxisRender {
    ...
    public void renderLabels(Canvas canvas) {
        ...
        for (int i = 0; i < labelCount; i++) {
          ...
          //  Log.e("AXTAG","原来的==="+label+"--新的==="+labeList.size());
            String labStr;
            if (labeList.size()>0){
                try {
                    if (i<entries.size()){
                        labStr = labeList.get(Integer.parseInt(label));
                       // Log.e("AXTAG",i+"原来==="+label+"--新==="+entries.get(i).getX());
                    }else {
                        labStr = "";
                    }

                 //   Log.e("AXTAG",i+"原来的==="+label+"--新的==="+labeList.get(i));
                }catch (Exception e){
                    labStr = "";
                }
                canvas.drawText(labStr, labelX, labelY, _PaintLabel);
            }else {
